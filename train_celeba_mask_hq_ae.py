    ...
    with torch.no_grad():
        ...
        for batch_idx, (images, masks, target) in enumerate(val_loader):
            ...
            if show and (np.random.uniform() < 0.2):
                ...
                plt.axis("off")
                plt.savefig('./images/celeba_mask/celeb_hq_mask_AE_' + '_dsize_' + str(res_size) + '_' + str(model.size_z) + '__' + str(epoch) + '__' + str(batch_idx) +  unq_name + '.png')
...
def run(res_size, epochs, batch_size, lr, size_z, beta, noise_cons, unq_name):
    ...
    for p in ['./models/celeba_mask', './plots/celeba_mask', './images/celeba_mask']:
    ...
    size_in = res_size
    img_ch = 1    
    mask_ae = ResAEN(enc_channel_list, dec_channel_list, size_in, size_z, img_ch)
    optimizer = torch.optim.Adam(mask_ae.parameters(), lr=lr)
    ...
    for epoch in range(epochs):
        ...
        training_loss = train_model(train_dataloader, mask_ae, optimizer, epoch, device, kl_cons, noise_cons)
        validation_loss = evaluate_model(val_dataloader, mask_ae, device, epoch, unq_name, res_size, show, kl_cons)
        ...
        if epoch > 0 and (validation_loss[0] < prev_loss):
            torch.save({
            'epoch': epoch,
            'model_state_dict': mask_ae.state_dict(),
            # 'optimizer_state_dict': optimizer.state_dict(),
            'train_loss': training_loss,
            'val_loss': validation_loss,
            'size_z': size_z,
            }, "./models/celeba_mask/celeb_hq_mask_AE_" + "_dsize_" + str(res_size) + "_z_" + str(size_z) + str(unq_name))
    ...
    mask_ae = ResAEN(enc_channel_list, dec_channel_list, size_in, size_z, img_ch)
    model_name = "./models/celeba_mask/celeb_hq_mask_AE_" + "_dsize_" + str(res_size) + "_z_" + str(size_z) + str(unq_name)
    mask_ae.load_state_dict(torch.load(model_name)['model_state_dict'])
    mask_ae = mask_ae.to(device)
    evaluate_model(val_dataloader, mask_ae, device, epoch, unq_name, res_size, show, kl_cons)

    train_losses, val_losses = np.array(train_losses), np.array(val_losses)
    save_loss_plot(train_losses, val_losses, \
        ['Total loss', 'Recon', 'KL'],
        ['Epoch']*3, 
        ['Total loss', 'Recon', 'KL'], './plots/celeba_mask/celeb_hq_mask_AE_' + "_dsize_" + str(res_size) + "__" + str(size_z) + unq_name + ".png")    
...
if __name__ == '__main__':
    ...
    parser.add_argument('--unq-name', type=str, default='_mask_celebhq_ae_',
                        help='identifier name for saving [default: "_mask_celebhq_ae_"]')
